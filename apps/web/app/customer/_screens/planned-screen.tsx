...
import { CUSTOMER_PLANNED } from './planned-content';
...
export function PlannedScreen({ route, title }: { route: string; title: string }) {
  const content = CUSTOMER_PLANNED[route];
  ...
  return (
    <>
      <PageHeader
        ...
        description={content?.does ?? 'This part of your account is being built.'}
      />
      ...
      >
        ...
        <p style={{ margin: 0, lineHeight: 1.7 }}>
          {content?.now ??
            'Everything available today is in the menu — your garage, reporting a problem, and tracking a repair.'}
        </p>
        {content?.href ? (
          <p style={{ margin: 0 }}>
