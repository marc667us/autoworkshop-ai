...
/**
 * Sign out of the customer workspace — T-0005 finding 5.
 *
...
export async function signOutAction(): Promise<never> {
  ...
  // on that page.
  //
  // `/customer` redirects into this pack's dashboard, which for a signed-out viewer
  // renders the shell with Sign in — the pre-merge behaviour, at the new path.
  return performSignOut(ARTIFACT_WORKSPACE, { returnTo: '/customer' });
}
