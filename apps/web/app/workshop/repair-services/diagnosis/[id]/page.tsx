...
/**
 * /repair-services/diagnosis/<id> — one diagnosis record. the §34 WORKSPACE DEFAULT route, seen by a role with no tree of its own: a platform administrator, cashier, storekeeper or quality-control inspector.
 *
 * ⚠️ GATED ON THE PARENT LIST ROUTE, not on this one. A detail route is DYNAMIC and no
 * navigation advertises one entry per diagnosis, so "is `/repair-services/diagnosis/<id>` in your menu"
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <DiagnosisSheetScreen route="/repair-services/diagnosis" diagnosisId={id} />;
}
