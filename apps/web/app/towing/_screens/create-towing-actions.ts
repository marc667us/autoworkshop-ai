...
/**
 * Register a towing company — the CALLER for `POST /registration/towing`.
 *
 * 🔴 WITHOUT THIS FILE THE ROUTE IS NOT SHIPPED. Migration 080 created
 * `identity.register_towing_operator` and the controller now exposes it, and neither
 * fact puts a towing operator in the database. `POST /registration/customer`
 ...
 */
export async function createTowingAction(formData: FormData): Promise<ActionResult> {
  const read = (k: string) => {
  ...
  const result = await apiPost<Created>('towing', '/registration/towing', {
    companyName,
      ...
      result.reason === 'invalid'
        ...
        : result.reason === 'unauthenticated'
          ...
          : result.reason === 'forbidden'
            ? 'This account may not register a towing company.'
  ...
  // answering, the navigation gains the towing tree, and the top bar can name
