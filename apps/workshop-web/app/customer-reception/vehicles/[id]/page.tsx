...
/**
 * /customer-reception/vehicles/<id> — detail, on the §34 default tree.
 *
 * ⚠️ GATED ON THE PARENT LIST ROUTE, not on this one. A detail route is DYNAMIC
 * and no navigation advertises one entry per record, so "is `/customer-reception/vehicles/<id>`
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <VehicleDetailScreen id={id} listHref="/customer-reception/vehicles" customerHrefBase="/customer-reception/customers" />;
}
