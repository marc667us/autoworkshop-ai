...
/**
 * /testing/road-test/<id> — one test session. the §49 TECHNICIAN tree — the same session, entered by its road test.
 *
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  await requireNavRoute('workshop', '/testing/road-test');
  const { id } = await params;
  return <TestingSheetScreen route="/testing/road-test" sessionId={id} />;
}
