...
/**
 * /record-work/repair-evidence/<id> — one repair. the §49 TECHNICIAN tree — the same repair record, entered by its evidence.
 *
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  await requireNavRoute('workshop', '/record-work/repair-evidence');
  const { id } = await params;
  return <ExecutionSheetScreen route="/record-work/repair-evidence" executionId={id} />;
}
