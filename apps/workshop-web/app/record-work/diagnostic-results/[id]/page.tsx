...
/**
 * /record-work/diagnostic-results/<id> — one diagnosis record. the §49 TECHNICIAN tree — their own assigned work, including any rejection they must act on.
 *
 * ⚠️ GATED ON THE PARENT LIST ROUTE, not on this one. A detail route is DYNAMIC and no
 * navigation advertises one entry per diagnosis, so "is `/record-work/diagnostic-results/<id>` in your menu"
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <DiagnosisSheetScreen route="/record-work/diagnostic-results" diagnosisId={id} />;
}
