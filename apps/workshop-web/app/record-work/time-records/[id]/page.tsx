...
/**
 * /record-work/time-records/<id> — one repair. the §49 TECHNICIAN tree — the same repair record, entered by its time.
 *
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  await requireNavRoute('workshop', '/record-work/time-records');
  const { id } = await params;
  return <ExecutionSheetScreen route="/record-work/time-records" executionId={id} />;
}
