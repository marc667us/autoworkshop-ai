...
/**
 * /repair-control/diagnosis/<id> — one diagnosis record. the §46 WORKSHOP OWNER tree.
 *
 * ⚠️ GATED ON THE PARENT LIST ROUTE, not on this one. A detail route is DYNAMIC and no
 * navigation advertises one entry per diagnosis, so "is `/repair-control/diagnosis/<id>` in your menu"
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  await requireNavRoute('workshop', '/repair-control/diagnosis');
  const { id } = await params;
  return <DiagnosisSheetScreen route="/repair-control/diagnosis" diagnosisId={id} />;
}
