-- The PUBLIC parts catalogue and mechanic directory (migration 021).
--
-- ⚠️ ONE FILE, TWO CALLERS, AND THAT IS THE POINT. `scripts/seed-dev-catalogue.sh`
-- runs it against the local container; `.github/workflows/seed-live-catalogue.yml`
-- runs the SAME statements against the deployed database. It was previously
-- five heredocs inside the dev script, which meant seeding production would
-- have required a second copy — and two copies of a catalogue drift until the
-- live shop and the local one disagree about what is for sale (§3: extend,
-- never duplicate).
--
-- ⚠️ EVERY BLOCK OPENS ITS OWN TRANSACTION AND SETS `app.current_role`.
-- `set_config(..., true)` is TRANSACTION-LOCAL and migration 021's `admin_write`
-- policy reads it. Without the explicit BEGIN each statement would be its own
-- transaction, the setting would be gone by the next one, and every INSERT
-- would be silently refused by RLS — zero rows, exit code 0.
--
-- ⚠️ IT DOES NOT RELY ON BEING A SUPERUSER. That matters more than it looks:
-- Render's database user is NOT one, so anything that leaned on superuser RLS
-- bypass works locally and fails live (see migration 037's write-up). This
-- takes the policy's own documented door instead.
--
-- ⚠️ IT SEEDS ONE UNPUBLISHED SUPPLIER AND ONE UNPUBLISHED PART ON PURPOSE.
-- With everything published, a broken `is_published` policy and a working one
-- render identically. Those two rows must NEVER appear on the public page.
--
-- ⚠️ CURRENCY IS EXPLICIT ON EVERY ROW. Migration 021 defaults the column to
-- 'GBP' while this application prices in GHS; 021 is checksummed so the default
-- cannot be edited in place. Relying on it would price a Ghanaian part in
-- sterling.
--
